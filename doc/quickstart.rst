...
.. note:: Do not use the path ``/etc/suricata`` and ``/var/lib`` for
	        example, since it could be any other path that user defines
	        during configuration and is by default ``/usr/local``. Please
	        check your configuration directory for the path and use that
	        accordingly.

Setup the directories with the correct permissions for the ``suricata``
group::

    sudo chmod -R g+rw /etc/suricata/
