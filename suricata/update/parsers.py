...
def parse_update(subparsers, global_parser, arguments):
...
def parse_arg():
    ...
    global_parser = parse_global(arguments)
    ...
    arguments = [
        (("-o", "--output"),
            {'metavar': '<directory>',
                'dest': 'output', 'help': "Directory to write rules to"}),
        (("-f", "--force"),
            {'action': 'store_true', 'default': False,
    ...
    update_parser = parse_update(subparsers, global_parser, arguments)
